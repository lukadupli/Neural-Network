...
int input_r = 2, input_c = 2, input_d = 2;
...
int main() {
	...
	Eigen::Tensor<double, 2> reshaped1 = patches.reshape(Eigen::array<ptrdiff_t, 2>({ patches_total / (kernel_r * kernel_c), kernel_r * kernel_c}));
	...
	Eigen::Tensor<double, 4> out = contraction.reshape(Eigen::array<ptrdiff_t, 4>{input_d, input_r, input_c, kernel_d});
